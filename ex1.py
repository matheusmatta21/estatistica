...
p = scp.shapiro(sync).pvalue #sync com ctz eh normal, ate pela tamanha discrepancia de 0.05 (default)
print(p)
pa = scp.shapiro(asyncr).pvalue
print(pa)

def teste_hipotese(p_value): return "Reiejtar H0" if p_value < 0.05 else "Nao rejeitar h0"

print(teste_hipotese(p))
print(teste_hipotese(pa))

print(scp.skew(sync)) #simetrico
print(scp.skew(asyncr)) #cauda a direita
print(scp.kurtosis(sync)) #curtose normal (0.4 ainda é bem proximo de 0)
print(scp.kurtosis(asyncr)) #curtose positiva

#variabiliadade: iqr, dp, variancia

#2, plotar o grafico, coeficiente e ver a correlacao
